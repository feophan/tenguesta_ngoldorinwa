...
1. `inye túle, apa isse úne` \[&mdash;\]. ***I*** *came, but* ***HE*** *didn’t*. All other alternatives (e.g. me not coming, or him coming) are explicitly dismissed.
...
1. `Elerondo quente insen` *Elrond said to himself*. Direct reflexive: `inse` is the indirect object of `quente`, and refers to its subject.
2. `Elerondo hlasse sa Elessar quente sen` *Elrond heard what Aragorn told him*. Indirect reflexive: `sen` refers back to the subject of `hlasse`, but syntactically is a part of the dependent clause.
...
1. The dogs<sub>1</sub> bit them<sub>2/\*1</sub>.
...
1. `Zindacollo`<sub>1</sub> `tatalla` `je`<sub>2</sub>/`se`<sub>1/2</sub>/`inse`<sub>1</sub>. *Thingol admires him*<sub>2</sub>/*him(self)*<sub>1/2</sub>/*himself*<sub>1</sub>.
...
1. `Telperinquar Erciandello tence sine tenguar` \[translated; LotR/305\]. *Celebrimbor of Hollin drew these signs*. `sine` refers to the signs actually present on the door, &rarr; we might imagine Celebrimbor pointing at them.
...
>[!NOTE]
>On impersonal constructions of exclusive generic statement (cf. E. *they*), &rarr;36.14. In colloquial speech `mo` can generally be replaced with inclusive 'we'.
